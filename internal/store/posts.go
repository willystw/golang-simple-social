...
func (s *PostStore) GetUserFeeds(ctx context.Context, userID int64, fq PaginatedFeedQuery) ([]PostWithMetadata, error) {
	query := `
		SELECT 
	...
	rows, err := s.db.QueryContext(ctx, query, userID, fq.Limit, fq.Offset, fq.Keyword, pq.Array(fq.Tags))
	if err != nil {
		return nil, err
	}
...
func (s *PostStore) GetByID(ctx context.Context, postID int64) (*Post, error) {
	query := `
		SELECT id,title,user_id,content,created_at,tags,updated_at,version FROM posts WHERE id= $1
	`
